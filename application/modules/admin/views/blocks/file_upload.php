        ...
        <tr class="template-download fade" id="{%=file.id%}">
            ...
            <td>
                {% if (file.url) { %}
                    ...
                    {% if (file.deleteUrl) { %}
                        <label class="labelFileState" for="active{%=file.id%}">Visible</label>
                        <input type="checkbox" class="fileState" name="name[{%=file.id%}]" id="active{%=file.id%}" data-id="{%=file.id%}" value="1" {% if (file.active == 1) { %} checked {% } %}/>
