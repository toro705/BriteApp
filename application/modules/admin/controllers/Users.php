...
class Users extends MX_Controller {
	...
	public function __construct()
	{
        ...
        $this->filesFolders     = [
            'upload'            => 'public/app/files/photographers/original/', 
            'large'             => 'public/app/files/photographers/large/',
            'medium'            => 'public/app/files/photographers/medium/',
            'small'             => 'public/app/files/photographers/small/',
            'crop'              => 'public/app/files/photographers/crop/'
        ];
    ...
    public function create($type = FALSE, $id = FALSE)
    {
        ...
        if ($type AND in_array($type, array(1, 2))) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($user->user_type_id == 2) {
                    redirect('admin/forums/redirect');
                } else {
                    $this->redirect($type);
                }
            }
    ...
    public function update_file_active()
    {
        if ($this->input->is_ajax_request()) {
            $data['success'] = FALSE;
            $input = $this->input->post();
            if ($this->db->where('id', $input['id'])->update('user_files', ['active' => $input['active']])) {
                $data['success'] = TRUE;
            }
            die(json_encode($data));
        } else {
