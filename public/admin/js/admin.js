var Admin = function () {
    ...
    return {
            ...
            videoAdd = function () {
                ...
                $(document).on('click', '#conceptAdd', function() {
                    ...
                    player.getCurrentTime().then(function(seconds) {
                        ...
                        '<button type="button" class="btn btn-warning btn-flat pull-right view"><i class="fa fa-eye"></i></button>'+
        ...
        cropImage: function() {
            ...
            $('.fancybox').fancybox({
                ...
                afterShow   : function() {
                    ...
                    $('.fancybox-overlay .actions').append('<div class="text-center mt-10"><a title="Cancelar" class="fancybox-item fancyboxClose btn btn-warning btn-flat" href="javascript:;">Cancelar</a><a title="Guardar" class="fancybox-item fancyboxSave btn btn-warning btn-flat" href="javascript:;">Guardar</a></div>');
                    /*
                     * -----------------------------------------------------------------
                     * Crop "Jcrop"
                     * -----------------------------------------------------------------
        ...
        dropDownChat: function() {
            $('.direct-chat-warning .box-header .direct-chat-msg').click(function (e) {
                        ...
                        if (result) {
                            ...
                            $.ajax({
                                ...
                                if (result && result.success && result.private == 1) {
                                    element.data('private', 0).removeClass('btn-warning').addClass('btn-success').text('Pública').removeAttr('title').attr('data-original-title', 'Cambiar estado a Pública');
        ...
        updateFileActive: function () {
            $(document).on('change', '.fileState', function () {
                var element = $(this),
                    id = element.data('id'),
                    active = (element.prop('checked') ? 1 : 0),
                    checked = (element.prop('checked') ? false : true);
                element.prop('checked', checked);
                $.ajax({
                    url: Admin.baseUrl(Admin.uriSegment(currentUriSegment) + '/update_file_active'),
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        active: active,
                    ...
                    bootbox.alert({
                        message: 'Lo sentimos, su registro no pudo ser actualizado, intente nuevamente.',
                        buttons: {
                            ok: {
                                label: 'Cerrar',
                                className: 'btn-warning btn-flat'
